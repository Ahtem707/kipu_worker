...
class DetailedTelegramCalendar(TelegramCalendar):
    ...
    def _build_days(self, *args, **kwargs):
        ...
        events = Events.select().where(Events.date.year == self.current_date.year,
                                       Events.date.month == self.current_date.month)
        start = self.current_date.replace(day=1)
        days = self._get_period(DAY, start, days_num)
        if len(events) > 0:
            days_buttons = rows(
                [
                    self._build_button(self._button_day(d.day, events) if d else self.empty_day_button, SELECT if d else NOTHING, DAY, d,
                                       is_random=self.is_random)
                    for d in days
                ],
                self.size_day
    ...
    def _button_day(self, day, events):
        if day in [event.date.day for event in events]:
            custom_day = ''
            for d in str(day):
                if d == '1':
                    custom_day += '❶'
